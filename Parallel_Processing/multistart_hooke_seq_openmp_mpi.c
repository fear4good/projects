...
//        MPI_Init(NULL, NULL);
	...
	#pragma omp parallel for shared(nvars) ,private(i)
	for (i = 0; i < nvars; i++) {
	...
	//	MPI_Bcast(&newf, nvars, MPI_INT, 0, MPI_COMM_WORLD);
	...
	//	MPI_Reduce(&delta, &fbefore, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

		if ((steplength >= epsilon) && (newf >= fbefore)) {
			steplength = steplength * rho;
			#pragma omp parallel for shared(nvars,rank,p),private(i,rho),reduction(*:delta)
			for (i = rank; i < nvars; i=i+p) {
				delta[i] *= rho;

			MPI_Reduce(&delta,&pi,1,MPI_DOUBLE,MPI_PROD,0,MPI_COMM_WORLD);
			}
		}
	}
//	MPI_Finalize();
